export const noDocumentMatchedCriteria =
	'Operation not performed.  No document matched the request criteria';

export const invalidSessionID = 'Invalid sessionID';
export const notLoggedIn = 'not logged in';
export const duplicateUser = 'Duplicate key error: E11000 duplicate key error collection:' +
	' rest-api.image-library-app-user index: email_1';
export const duplicateLibrary = 'Duplicate key error: E11000 duplicate key error collection:';
